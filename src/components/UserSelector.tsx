      ...
      >
        {selectedUser ? (
          <>
            ...
            <span className="text-foreground truncate text-sm">{selectedUser.name}</span>
          </>
        ) : (
          <span className="text-muted-foreground/40">Select user…</span>
        )}
        {selectedUser && (
          <button
            type="button"
        ...
        <ChevronDown className={`h-3.5 w-3.5 text-muted-foreground/50 ${selectedUser ? '' : 'ml-auto'} flex-shrink-0 transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
      </button>

      {open && (
        <div className="absolute top-full left-0 right-0 mt-1.5 z-[100] rounded-lg overflow-hidden border border-border/50 shadow-2xl animate-fade-in bg-background/97 backdrop-blur-3xl" style={{ WebkitBackdropFilter: 'blur(60px)', backdropFilter: 'blur(60px)' }}>
